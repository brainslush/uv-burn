...
def convert_pyprojects_to_pipfile(pyprojects: list[PyProject]) -> tuple[Pipfile, dict[Url, str]]:
    ...
    python_version = pyprojects[0].project.requires_python
...
def convert_uv_lock_to_pipfile_lock(
    ...
    for package in uv_lock.packages:
        ...
        required_python_marker = f"python_version{required_python}"

        default_packages[package.name] = LockPackageEntry(
            version=package.version,
            hashes=hashes,
            index=source_url_name_map[package.source.registry],
            markers=" and ".join([required_python_marker, *markers[package.name]]),
        )

    return PipfileLock(
        meta=LockMeta(
            hash=LockHash(sha256=pipfile_hash),
            requires=LockMetaRequires(python_version=uv_lock.requires_python),
            sources=[
                LockMetaSource(
                    name=name,
                    url=url,
